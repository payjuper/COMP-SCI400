...
public class Backend_Placeholder implements BackendInterface {
  ...
  public List<String> getListOfAllLocations() {
    // the original placeholder expected the graph to offer getAllNodes()
    // but our Graph_Placeholder doesn't declare that in the interface,
    // so we check and downcast here
    if (graph instanceof Graph_Placeholder) {
      return ((Graph_Placeholder)graph).getAllNodes();
    }
    // fallback so this still compiles even with other graphs
  ...
  public List<String> getLongestLocationListFrom(String startLocation) throws NoSuchElementException {
    // same issue as above: use the helper only if the graph is our placeholder
    List<String> all;
    if (graph instanceof Graph_Placeholder) {
      all = ((Graph_Placeholder)graph).getAllNodes();
    } else {
      all = new ArrayList<>();
    }
    if (all.isEmpty())
